...
bool ChatHandler::HandleJailCommand(const char *args)
{
    ...
    if (!chr)
    {
        ...
        CharacterDatabase.BeginTransaction();
        if (!result) CharacterDatabase.PExecute("INSERT INTO `jail` VALUES ('%u','%s','%u','%s','%u','%u','%s',CURRENT_TIMESTAMP,'%u')", jail_guid, jail_char.c_str(), jail_release, jail_reason.c_str(), jail_times, jail_gmacc, jail_gmchar.c_str(), jailtime);
